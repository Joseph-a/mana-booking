...
jQuery( document ).ready( function () {
	...
	var mainBookingForm    = jQuery( '.main-availability-form' ),
	...
	var roomBookingForm     = jQuery( '.room-booking-form-container' ),
	...
	jQuery( '#login-form-url, #login-register-url, .booking-more-info-btn' ).magnificPopup( {
		type:         'inline',
		preloader:    false,
		mainClass:    'mfp-fade',
		removalDelay: 600
	...
	loginFrom.on( "submit", function ( e ) {
		...
		jQuery.ajax( {
			type:     'POST',
			dataType: 'json',
			url:      ravis_booking_front.ajaxurl,
			data:     {
				'action':   'ravis_booking_login',
			...
			success:  function ( data ) {
				...
				{
					document.location.href = ravis_booking_front.redirecturl;
				}
				else
				{
	...
	profileForm.on( 'submit', function ( e ) {
		...
		} else
		{
			jQuery.ajax( {
				...
				url:      ravis_booking_front.ajaxurl,
				data:     {
					'action':   'ravis_booking_update_profile',
					'fName':    fName,
					'lName':    lName,
					'phone':    phone,
					'email':    email,
	...
	if ( jQuery.isFunction( jQuery.fn.barrating ) )
	{
		jQuery( ".rate-items" ).barrating( "show", {
			onSelect: function ( value, text, event ) {
				var _this       = jQuery( event.srcElement ),
	...
	if ( currencySwitchers.length > 0 )
	{
		currencySwitchers.on( 'change', function () {
			var _this    = jQuery( this ),
				postData = {
					action:   "ravis_booking_currency_cookie",
					currency: _this.val()
				};
			jQuery.ajax( {
				url:      ravis_booking_front.ajaxurl,
