<?php
	...
	class Ravis_booking_insert_post
	{
		...
		public static function insert_property()
		{
			...
			if ( ! empty( $data_fields->p_title ) && ! empty( $data_fields->p_type ) && ! empty( $data_fields->p_status ) && ! empty( $data_fields->p_bedrooms ) && ! empty( $data_fields->p_bath ) && ! empty( $data_fields->p_build_size ) && ! empty( $data_fields->p_lot_size ) && ! empty( $data_fields->p_desc ) && ! empty( $data_fields->p_price ) && ! empty( $data_fields->p_address ) && ! empty( $data_fields->p_agent ) && ! empty( $data_fields->p_agreement ) )
			{
				...
				$response['text']   = esc_html__( 'Thanks for submission, your property will be available after confirmation of administration of website.', 'ravis-booking' );
			}
			else
			{
				$response['status']  = false;
				$response['errorNo'] = 1;
				$response['text']    = esc_html__( 'Please fill all the required fields', 'ravis-booking' );
			}
		...
		public static function realty_house_register_agent()
		{
			...
			if ( ! empty( $data_fields->fname ) && ! empty( $data_fields->lname ) && ! empty( $data_fields->user_name ) && ! empty( $data_fields->email ) && ! empty( $data_fields->confirm_email ) && ! empty( $data_fields->address ) )
			{
				if ( $data_fields->email !== $data_fields->confirm_email )
				{
					$response['status']  = false;
					$response['errorNo'] = 2;
					$response['text']    = esc_html__( 'Email and its confirmation fields are not the same', 'ravis-booking' );
				}
				...
				if ( $approved_agents->post_count === 0 )
				{
					...
					else
					{
						$response['status']  = false;
						$response['errorNo'] = 1;
						$response['text']    = esc_html__( 'Your account is pending now, please be patient to be approved.', 'ravis-booking' );
					}
				}
				else
				{
					$response['status']  = false;
					$response['errorNo'] = 1;
					$response['text']    = esc_html__( 'You have already registered. Please try another Username or Email.', 'ravis-booking' );
				}
			}
			else
			{
	...
	$ravis_booking_insert_post_obj = new Ravis_booking_insert_post();