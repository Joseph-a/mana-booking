<?php
    ...
    class Ravis_booking_booking_process
    {
        public $ravis_booking_option;
        public $ravis_booking_param;

        public function __construct()
        {
            $this->ravis_booking_option = get_option('ravis-booking-setting');
            $booking_param              = (!empty($this->ravis_booking_option['booking_url']) ? $this->ravis_booking_option['booking_url'] : 'ravis-booking');
            $this->ravis_booking_param  = $booking_param;
            ...
            add_action('wp_ajax_nopriv_ravis_booking_check_availability', array($this, 'check_availability'));
            add_action('wp_ajax_ravis_booking_check_availability', array($this, 'check_availability'));

            add_action('wp_ajax_nopriv_ravis_booking_insert_booking', array($this, 'insert_booking'));
            add_action('wp_ajax_ravis_booking_insert_booking', array($this, 'insert_booking'));

            add_action('wp_ajax_nopriv_ravis_booking_update_booking_status', array($this, 'update_status'));
            add_action('wp_ajax_ravis_booking_update_booking_status', array($this, 'update_status'));

            add_action('wp_ajax_nopriv_ravis_booking_delete_booking', array($this, 'delete_booking'));
            add_action('wp_ajax_ravis_booking_delete_booking', array($this, 'delete_booking'));

            add_action('wp_ajax_nopriv_ravis_booking_booking_overview', array($this, 'booking_overview'));
            add_action('wp_ajax_ravis_booking_booking_overview', array($this, 'booking_overview'));

            add_action('wp_ajax_nopriv_ravis_booking_room_overview', array($this, 'room_overview'));
            add_action('wp_ajax_ravis_booking_room_overview', array($this, 'room_overview'));

            add_action('wp_ajax_nopriv_ravis_booking_update_invoice', array($this, 'update_invoice'));
            add_action('wp_ajax_ravis_booking_update_invoice', array($this, 'update_invoice'));

            add_action('wp_ajax_nopriv_ravis_booking_stripe_invoice', array($this, 'stripe_invoice'));
            add_action('wp_ajax_ravis_booking_stripe_invoice', array($this, 'stripe_invoice'));
        }

        public function is_booking()
        {
            if (isset($_GET[ $this->ravis_booking_param ]) || isset($_GET['booking-notification']))
            {
                return true;
            }
            else
        ...
        {
            add_rewrite_rule('booking.php$', 'index.php?' . esc_url($this->ravis_booking_param) . '=1', 'top');
        ...
        {
            $query_vars[] = $this->ravis_booking_param;
        ...
        {
            if (array_key_exists($this->ravis_booking_param, $wp->query_vars))
            {
                include RAVIS_BOOKING_TPL . 'booking.php';
                exit();
            }
            if (array_key_exists('booking-notification', $wp->query_vars))
            {
                include RAVIS_BOOKING_TPL . 'booking-notification.php';
                exit();
            }
        ...
        {
            global $wpdb;
            if (check_ajax_referer('ravis-booking-security-str', 'security'))
            {
                $check_in       = !empty($_REQUEST['checkIn']) ? sanitize_text_field($_REQUEST['checkIn']) : '';
                $check_out      = !empty($_REQUEST['checkOut']) ? sanitize_text_field($_REQUEST['checkOut']) : '';
                $adult          = !empty($_REQUEST['adult']) ? intval($_REQUEST['adult']) : '';
                $child          = !empty($_REQUEST['child']) ? intval($_REQUEST['child']) : '';
                ...
                $selected_rooms = !empty($_REQUEST['rooms']) ? sanitize_text_field($_REQUEST['rooms']) : '';
                ...
                if (!empty($check_in) && !empty($check_out) && $total_guests !== 0)
                {
                    ...
                    $block_date_args  = array(
                        ...
                        'meta_query'  => array(
                            ...
                            array(
                                'relation' => 'AND',
                                array(
                                    'key'     => 'ravis_booking_block_dates_from',
                                    'value'   => $check_in,
                                    'compare' => '<='
                                ),
                                array(
                                    'key'     => 'ravis_booking_block_dates_to',
                                    'value'   => $check_in,
                                    'compare' => '>='
                                )
                            ),
                    ...
                    if ($room_list_query->have_posts())
                    {
                        while ($room_list_query->have_posts())
                        {
                            ...
                            $room_id   = Ravis_booking_get_info::original_post_id(get_the_ID());
                    ...
                    else
                    {
                        ...
                        $return_value['message'] = esc_html__('No Room Available', 'ravis-booking');
                    }
                ...
                {
                    $return_value['status']  = false;
                    $return_value['message'] = esc_html__('Please choose the correct information.', 'ravis-booking');
            ...
            {
                $return_value['status']  = false;
                $return_value['message'] = esc_html__('Your are cheating!', 'ravis-booking');
        ...
        public function insert_booking()
        {
            ...
            {
                $room_id_str .= Ravis_booking_get_info::original_post_id($room_obj->roomID) . ',';
            }
            ...
            if (check_ajax_referer('ravis-booking-security-str', 'security'))
            {
                if (!empty($booking_info_obj->fname) && !empty($booking_info_obj->lname) && !empty($booking_info_obj->email) && !empty($booking_info_obj->phone) && !empty($booking_info_obj->terms) && !empty($booking_info_obj->checkIn) && !empty($booking_info_obj->checkOut))
                {
                    ...
                    {
                        $return_value['status']  = false;
                        $return_value['message'] = esc_html__('You have already booked, after checking your information, your booking will be finalized.', 'ravis-booking');
                    ...
                    else
                    {
                        ...
                        if ($inserted_booking)
                        {
                            ...
                            {
                                case 'paypal':
                                    if (!empty($this->ravis_booking_option['paypal_booking']))
                                    {
                                        $payable_price = $booking_info_obj->totalBookingPrice;
                                        if ($booking_info_obj->paymentPriceMethod === '2' && !empty($this->ravis_booking_option['deposit_status']))
                                        {
                                            $user_deposit  = !empty($this->ravis_booking_option['deposit']) ? esc_html($this->ravis_booking_option['deposit']) : 20;
                                        ...
                                        if ($inserted_invoice)
                                        {
                                            ...
                                            $paypal_default_currency = $this->ravis_booking_option['paypal_default_currency'];
                                            ...
                                            $return_value['paymentForm'] = '
												<form id="paypal-form" method="post" action="' . (!empty($this->ravis_booking_option['paypal_action_url']) ? esc_html($this->ravis_booking_option['paypal_action_url']) : '') . '">
													<input type="hidden" name="business" value="' . (!empty($this->ravis_booking_option['paypal_email']) ? esc_html($this->ravis_booking_option['paypal_email']) : '') . '">
													<input type="hidden" name="cmd" value="_xclick">
													<input type="hidden" name="item_name" value="' . esc_html__('Room Booking Invoice', 'ravis-booking') . '">
                                        ...
                                        {
                                            $return_value['status']  = false;
                                            $return_value['message'] = esc_html__('Because of some technical issues, your booking was not proceed. Please try again.', 'ravis-booking');
                                        }
                                        echo json_encode($return_value);
                                        die();
                                    }
                                    ...
                                    {
                                        $return_value['status']  = false;
                                        $return_value['message'] = esc_html__('Paypal payment is disabled on your website.', 'ravis-booking');
                                    ...
                                    break;
                                case 'paymill':
                                    if (!empty($this->ravis_booking_option['paymill_booking']))
                                    {
                                        $payable_price = $booking_info_obj->totalBookingPrice;
                                        if ($booking_info_obj->paymentPriceMethod === '2' && !empty($this->ravis_booking_option['deposit_status']))
                                        {
                                            ...
                                            $return_value['paymentForm'] = '
												<form id="paymill-form" action="#" method="POST" data-invoice="' . esc_attr($invoice_id) . '" data-ajax-url="' . esc_url(admin_url('admin-ajax.php')) . '">
													...
													<div class="field-row clearfix">
														<div class="col-md-12">
															<input class="card-number" type="text" size="20" placeholder="' . esc_html__('Card number', 'ravis-booking') . '"/>
														</div>
													</div>
													<div class="field-row clearfix">
														<div class="col-md-12">
															<input class="card-cvc" type="text" size="4" placeholder="' . esc_html__('CVC', 'ravis-booking') . '"/>
														</div>
													</div>
													<div class="field-row clearfix">
													    <div class="col-md-6">
															<input class="card-expiry-month" type="text" size="2" placeholder="' . esc_html__('Expiry date (MM)', 'ravis-booking') . '"/>
														</div>
														<div class="col-md-6">
															<input class="card-expiry-year" type="text" size="4" placeholder="' . esc_html__('Expiry date (YYYY)', 'ravis-booking') . '"/>
														</div>
													</div>
													<div class="payment-errors"></div>
													<div class="field-row btn-container clearfix">
														<button class="submit-button" type="submit">' . esc_html__('Submit Payment', 'ravis-booking') . '</button>
													</div>
												</form>
												<script type="text/javascript">
													var PAYMILL_PUBLIC_KEY = "' . (!empty($this->ravis_booking_option['paymill_public_key']) ? esc_js($this->ravis_booking_option['paymill_public_key']) : '') . '";
													jQuery(function($){
                                    ...
                                    break;
                                case 'stripe':
                                    if (!empty($this->ravis_booking_option['stripe_booking']))
                                    {
                                        $payable_price = $booking_info_obj->totalBookingPrice;
                                        if ($booking_info_obj->paymentPriceMethod === '2' && !empty($this->ravis_booking_option['deposit_status']))
                                        {
                                        ...
                                        if ($inserted_invoice)
                                        {
                                            ...
                                            $return_value['paymentForm'] = '
												<form id="stripe-form" action="#" method="POST" data-invoice="' . esc_attr($invoice_id) . '" data-ajax-url="' . esc_url(admin_url('admin-ajax.php')) . '">
													...
													<input class="card-currency" type="hidden" value="' . esc_attr($current_currency['title']) . '" />
													<div class="field-row clearfix">
														<label for="card-element">' . esc_html__('Credit or Debit Card', 'ravis-booking') . '</label>
												...
												<script type="text/javascript">
													var stripe = Stripe("' . (!empty($this->ravis_booking_option['stripe_publish_key']) ? esc_js($this->ravis_booking_option['stripe_publish_key']) : '') . '");
                                ...
                                default:
                                    $return_value['status']  = true;
                                    $return_value['message'] = esc_html__('Thanks for your booking, after checking your information, your booking will be finalized.', 'ravis-booking');
                                    break;
                            }
                        ...
                        {
                            $return_value['status']  = false;
                            $return_value['message'] = esc_html__('Because of some technical issues, your booking was not successful, please try again.', 'ravis-booking');
                ...
                {
                    $return_value['status']  = false;
                    $return_value['message'] = esc_html__('Please provide all the required information.', 'ravis-booking');
        ...
        {
            $status = false;
            if (!empty($this->ravis_booking_option['email_notification_status']))
            {
                $currency_obj     = new Ravis_booking_currency();
                $get_info_obj     = new Ravis_booking_get_info();
                $user_deposit     = $this->ravis_booking_option['deposit'];
                $user_deposit_val = ($booking_info_obj->totalBookingPrice * $user_deposit) / 100;

                if (defined('ICL_LANGUAGE_CODE') && !empty($language))
                {
                    $room_tbl_title      = apply_filters('wpml_translate_single_string', 'Room Title', 'ravis-booking', 'room_tbl_title', $language);
                    $room_tbl_adult      = apply_filters('wpml_translate_single_string', 'Adult', 'ravis-booking', 'room_tbl_adult', $language);
                    $room_tbl_child      = apply_filters('wpml_translate_single_string', 'Child', 'ravis-booking', 'room_tbl_child', $language);
                    $room_tbl_price      = apply_filters('wpml_translate_single_string', 'Price', 'ravis-booking', 'room_tbl_price', $language);
                    $room_tbl_serv_title = apply_filters('wpml_translate_single_string', 'Title', 'ravis-booking', 'room_tbl_serv_title', $language);
                }
                else
                {
                    $room_tbl_title      = esc_html__('Room Title', 'ravis-booking');
                    $room_tbl_adult      = esc_html__('Adult', 'ravis-booking');
                    $room_tbl_child      = esc_html__('Child', 'ravis-booking');
                    $room_tbl_price      = esc_html__('Price', 'ravis-booking');
                    $room_tbl_serv_title = esc_html__('Title', 'ravis-booking');
                }
                $room_tmpl        = '
					<table>
						<tr>
                ...
                $email_sender = (!empty($this->ravis_booking_option['email_sender']) ? $this->ravis_booking_option['email_sender'] : get_option('admin_email'));
                ...
                {
                    case('guest'):
                        $body   = str_replace($link_shortcodes, $link_replace_text, $this->ravis_booking_option['email_user_tmpl']);
                        $subj   = esc_html__('Your Booking has been confirmed.', 'ravis-booking');
                        $status = wp_mail($booking_info_obj->email, $subj, $body, $headers);
                        break;
                    case('admin'):
                        $body                = str_replace($link_shortcodes, $link_replace_text, $this->ravis_booking_option['email_admin_tmpl']);
                        $multiple_recipients = (!empty($this->ravis_booking_option['email_receiver']) ? $this->ravis_booking_option['email_receiver'] : '');
                        $subj                = esc_html__('New Booking Information was in your website', 'ravis-booking');

                        if (!empty($this->ravis_booking_option['guest_receiver']) && !empty($booking_info_obj->email))
                        {
        ...
        public function booking_overview()
        {
            ...
            if (!empty($booking_result))
            {
                ...
                foreach ($booking_result as $booking_item)
                {
                    ...
                    if ($total_guest > 1)
                    {
                        $guest_txt = $total_guest . ' ' . esc_html__('guests', 'ravis-booking');
                    }
                    else
                    {
                        $guest_txt = $total_guest . ' ' . esc_html__('guest', 'ravis-booking');
                    }
                    $box_title = esc_html__('Rooms :', 'ravis-booking') . trim($room_str, ',');
                    if ($front_end === false)
                    {
        ...
        public function room_overview()
        {
            ...
            $room_count      = get_post_meta($room_id, 'ravis_booking_room_count', true);
            $table_name      = $wpdb->prefix . 'ravis_booking';
            ...
            while ($start_date_time < $end_date_time)
            {
                ...
                if ($booking_result >= $room_count || in_array($start_date_time, $block_data_array))
                {
                    $result[] = array(
                        'id'        => $i,
                        'title'     => esc_html__('Not Available', 'ravis-booking'),
                ...
                else
                {
                    if ($start_date_time > $today['sec'])
                    {
                        ...
                        {
                            $price_title = esc_html__('From', 'ravis-booking') . ' : ' . $room_info['booking_price']['total']['weekend']['adult']['main'];
                        }
                        else
                        {
                            $price_title = esc_html__('From', 'ravis-booking') . ' : ' . $room_info['booking_price']['total']['weekday']['adult']['main'];
                        }
        ...
        public function stripe_invoice()
        {
            ...
            if (!empty($token) && !empty($invoice_id) && wp_verify_nonce($security, 'stripe_form_security'))
            {
                ...
                \Stripe\Stripe::setApiKey($this->ravis_booking_option['stripe_secret_key']);
                try
                {
    ...
    $ravis_bookingBooking_process = new Ravis_booking_booking_process();
