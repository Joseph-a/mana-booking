<?php
    ...
    $ravis_booking_option = get_option('ravis-booking-setting');
    $per_page             = (!empty($ravis_booking_option['booking_archive_per_page']) ? intval($ravis_booking_option['booking_archive_per_page']) : 10);
    ...
    $current_page_url     = '//' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . '?page=ravis-booking-archive';
    $currency_obj         = new Ravis_booking_currency();
?>
<div id="ravis-booking-booking-archive">
	<div class="wrap">
		<h1><?php esc_html_e('Booking Archive', 'ravis-booking') ?></h1>
		<table class="wp-list-table widefat fixed" id="booking-archive-tbl">
			<thead>
			<tr>
				<th class="row-number">#</th>
				<th class="booking-id"><?php esc_html_e('ID', 'ravis-booking') ?></th>
				<th class="booking-name"><?php esc_html_e('Name', 'ravis-booking') ?></th>
				<th class="booking-phone"><?php esc_html_e('Phone', 'ravis-booking') ?></th>
				<th class="booking-email"><?php esc_html_e('Email', 'ravis-booking') ?></th>
				<th class="booking-checkin"><?php esc_html_e('Check In', 'ravis-booking') ?></th>
				<th class="booking-checkout"><?php esc_html_e('Check Out', 'ravis-booking') ?></th>
				<th class="booking-more"><?php esc_html_e('More Info', 'ravis-booking') ?></th>
				<th class="booking-status"><?php esc_html_e('Status', 'ravis-booking') ?></th>
				<th class="booking-actions"><?php esc_html_e('Actions', 'ravis-booking') ?></th>
			</tr>
			</thead>
			<tbody id="the-list">
			<?php
                    ...
                    foreach ($user_bookings as $booking_item)
                    {
                        ...
                        echo '
							<tr class="booking-' . esc_attr($booking_item->id) . '">
								...
								<td class="booking-more">' . esc_html__('More Details', 'ravis-booking') . '<i class="dashicons dashicons-arrow-down-alt2"></i></td>
								<td class="booking-status update-status-item ' . esc_attr($status_row_class) . '" data-id="' . esc_attr($booking_item->id) . '" data-lang="' . (!empty($booking_lang) ? esc_attr($booking_lang) : '') . '" data-nonce="' . wp_create_nonce('booking-update-status') . '">
									<i class="dashicons dashicons-yes" title="' . esc_html__('Confirmed', 'ravis-booking') . '"></i>
									<i class="dashicons dashicons-marker" title="' . esc_html__('Not Confirmed', 'ravis-booking') . '"></i>
								</td>
								<td class="booking-actions">
									<a data-id="' . esc_attr($booking_item->id) . '" class="print-item" href="#"><img src="' . esc_url(RAVIS_BOOKING_IMG_PATH) . 'print.png" alt="' . esc_html__('Print', 'ravis-booking') . '"></i></a>
									...
									<div class="more-detail-content">
										<div class="booking-main-info clearfix">
											...
											<div class="info-row row clearfix">
												...
												<div class="info-item total-price">
													<div class="title">' . esc_html__('Coupon Discount : ', 'ravis-booking') . '</div>
						...
						if (!empty($booking_info_obj->couponInfo))
                        {
                            echo '
										<div class="section-box">
											<div class="box-title">
												<span>' . esc_html__('Coupon Information', 'ravis-booking') . '</span>
											</div>
											<table>
												<tr>
													<th>' . esc_html__('Title', 'ravis-booking') . '</th>
													<th>' . esc_html__('Type', 'ravis-booking') . '</th>
													<th>' . esc_html__('Price', 'ravis-booking') . '</th>
												</tr>
												<tr>
